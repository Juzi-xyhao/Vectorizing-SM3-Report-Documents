...
public final class SM3Engine implements Cloneable {
    ...
    private void processWord() {
        ...
        block[blockOffset]
                ...
                | ((word[2] & 0xFF) << 8)
                | ((word[3] & 0xFF));
        wordOffset = 0;  // 处理完了word数组，重置其偏移量
        blockOffset++;  //增加blockOffset，表示block数组中的下一个位置可用于存储新的整数

        if (blockOffset >= SM3_BLOCK_INT_SIZE) {   // 如果blockOffset达到16，这意味着已经填满了整个消息块，因此调用processBlock()方法来处理这个消息块。
            processBlock();
        }
    }
